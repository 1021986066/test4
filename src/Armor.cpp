...
bool Armor::fastExplore(Mat& frame)
{
    lights.clear();
    areas.clear();
    if(fastSelectContours(frame) == false)
        return false;
    return fastPairContours();
}

bool Armor::fastSelectContours(Mat& frame)
{
    static Mat bin;
    threshold(frame, bin, GRAY_THRESH, 255, THRESH_BINARY);
#if DRAW == SHOW_ALL
...
{
    if (slowSelectContours(frame) == false)
        return false;
    return slowPairContours();
}

bool Armor::slowSelectContours(Mat& frame)
{
    static Mat bin;
    threshold(frame, bin, GRAY_THRESH, 255, THRESH_BINARY);
#if DRAW == SHOW_ALL
